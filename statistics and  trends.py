...
Original file is located at
    https://colab.research.google.com/drive/17qhlSnmZTC_4hbFeRTeGrdr0uUnnNFrC
...
"""### Get data of urban_population over the years"""

# we want to see countries co2_emissions over specfic years
# we need to filter our original data frame to get specific fields

# Filter the data for non-null values
co2_emissions = org_df[['country', 'year', 'co2_emissions']].dropna()
...
countries = co2_emissions.country.unique()
...
for i, year in enumerate(years_to_plot):
    data = co2_emissions[co2_emissions['year'] == year]
    plt.bar(np.arange(data.shape[0]) + (0.2 * i), data['co2_emissions'], color=selected_colors[i], width=barWidth, label=str(year))
...
"""### Making a DataFrame related to China"""

# making dataframe of China data from the original dataframe
chn = org_df[org_df['country'] == 'China']
...
"""### For the Features Present In Russian Federation DataFrame remove the null values
...
columns_of_interest = ['co2_emissions', 'greenhouse_gas_emissions',
       ...
       'urban_population', 'nitrous_oxide', 'agricultural_land',
       'rural_population']
...
for column in columns_of_interest:
    feature_data[column] = remove_null_values(chn[[column]])
...
correlation_matrix = chn_clean_data.corr()
...
"""### Making a DataFrame related to Kenya"""

# making dataframe of Kenya data from the original dataframe
ken = org_df[org_df['country'] == 'Kenya']
...
# Loop through each column to extract and clean the data
for column in columns_of_interest:
    feature_data[column] = remove_null_values(ken[[column]])
    print(f'{column} Length = {len(feature_data[column])}')
...
# Create a correlation matrix
correlation_matrix = ken_clean_data.corr()

# Create a heatmap using Seaborn
plt.figure(figsize=(10, 8))
sns.heatmap(correlation_matrix, annot=True, cmap='Blues', fmt=".2f")
plt.title('Correlation Heatmap for Kenya')
plt.show()
...
"""### Making a DataFrame related to Saudi Arabia"""

# making dataframe of Saudi Arabia data from the original dataframe
sa = org_df[org_df['country'] == 'Saudi Arabia']
...
# Loop through each column to extract and clean the data
for column in columns_of_interest:
    feature_data[column] = remove_null_values(sa[[column]])
    print(f'{column} Length = {len(feature_data[column])}')
...
# Create a correlation matrix
correlation_matrix = sa_clean_data.corr()

# Create a heatmap using Seaborn
plt.figure(figsize=(10, 8))
sns.heatmap(correlation_matrix, annot=True, cmap='Greens', fmt=".2f")
plt.title('Correlation Heatmap for Saudi Arabia')
plt.show()
...
"""### Get the Year, Country Data Related to GDP"""

# we want to see countries GDP over the years
# we need to filter our original data frame to get specific fields
GDP = org_df[['country','year','GDP']]
...
for country in countries:
    ...
    plt.plot(country_data['year'], country_data['agricultural_land'], label=country)
...
nitrous_oxide_1990 = nitrous_oxide[nitrous_oxide['year'] == 1990]

# filter data related to 2010
nitrous_oxide_2010 = nitrous_oxide[nitrous_oxide['year'] == 2010]

# filter data related to 2020
nitrous_oxide_2020 = nitrous_oxide[nitrous_oxide['year'] == 2020]
